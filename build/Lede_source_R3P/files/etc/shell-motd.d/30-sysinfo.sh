...
# cpu info
sys_temp=$(cat /proc/cpuinfo | grep machine | cut -f2 -d:)
sys_tempx=`echo $sys_temp | sed 's/ / /g'`

# display info

printf "MACHINE   :    \x1B[92m%s\x1B[0m" "$sys_tempx"
echo ""

display "MEM  USAGE" "$memory_usage" "90" "1" "%" " of ${memory_total}M"
display "LOAD   AVG" "${load%% *}" "${critical_load}" "0" "" "${load#* }"
echo "" # fixed newline

display "BOOT USAGE" "$boot_usage" "90" "1" "%" " of $boot_total"
printf "UP    TIME:    \x1B[92m%s\x1B[0m\t\t" "$time"
echo "" # fixed newline

display "ROOT USAGE" "$root_usage" "90" "1" "%" " of $root_total"
printf "IP ADDRESS:    \x1B[92m%s\x1B[0m" "$ip_address"
echo "" # fixed newline

display "SWAP USAGE" "$swap_usage" "10" "0" "%" " of $swap_total""M"
